...
function minify(html, config, cssFiles) {
  const { root, filter_optimize } = config;
  const { inlines, delivery } = filter_optimize.css;
  html = html.replace(/<link rel="stylesheet"[^>]*\shref="([^"]+)"[^>]*>/ig, (match, href) => {
...
module.exports = function() {
  ...
  return Promise.map(inlines, path => {
    ...
    return streamRead(stream).then(str => {
      return {
        path: str
      };
    });
    ...
    return Promise.map(htmls, path => {
      ...
      return streamRead(stream)
        .then(str => {
          str = minify(str, config, cssFiles);
